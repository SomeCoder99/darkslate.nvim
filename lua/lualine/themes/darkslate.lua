...
local b = opts.plugin.lualine.brightness
local a_bg, b_bg, c_bg, a_fg, b_fg, c_fg, mode_fg
local norm, command, insert, visual, terminal, replace = "green", "yellow", "blue", "purple", "cyan", "red"
if b < -1 then
  norm = "bg_" .. norm
  command = "bg_" .. command
  insert = "bg_" .. insert
...
elseif b == -1 then
  ...
  mode_fg = "dark11"
  a_bg, b_bg, c_bg, a_fg, b_fg, c_fg = 3, 2, 1, 13, 12, 11
else
  norm = norm .. b
  command = command .. b
  insert = insert .. b
...
local theme = {
  normal = {
    a = { fg = c[mode_fg], bg = c[norm], gui = "bold" },
